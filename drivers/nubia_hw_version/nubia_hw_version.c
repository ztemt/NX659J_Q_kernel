...
static int  nubia_parse_hw_ver_gpio_dt(struct device_node *node,int* pcb_gpio1_v, int* pcb_gpio2_v)
{
	...
	*pcb_gpio2_v = pcb_gpio2;
	...
	nubia_pcb_gpio3_v = of_get_named_gpio(node, "qcom,pcb-gpio3", 0);
	if (!gpio_is_valid(nubia_pcb_gpio3_v)){
		nubia_hw_version_debug("TLMM pcb gpio3 not found\n");
		//return -EPROBE_DEFER;
		return 0;
	}

	rc = gpio_request(nubia_pcb_gpio3_v, "NUBIA_HW_PCB_GPIO3");
	if (rc < 0){
		printk("Failed to request GPIO:%d, ERRNO:%d\n", (s32)nubia_pcb_gpio3_v, rc);
		rc = -ENODEV;
	}
