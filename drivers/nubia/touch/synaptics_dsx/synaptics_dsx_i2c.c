...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	pr_err("[TP]%s: synaptics pwr-reg-name = %s\n", __func__, bdata->pwr_reg_name );
    ...
    //retval = regulator_enable((bdata->bus_reg_name));
	//pr_err("[TP]%s: synaptics retval = %d\n", __func__,retval );
	/*prop = of_find_property(np, "synaptics,bus-reg-name", NULL);
	if (prop && prop->length) {
		bdata->power_gpio = of_get_named_gpio_flags(np,
				"synaptics,bus-reg-name", 0, NULL);
	...
	pr_err("[TP]%s: synaptics,power-on-state = %d ,bdata->power_gpio:%d\n", __func__, bdata->power_on_state,bdata->power_gpio);
	*/
